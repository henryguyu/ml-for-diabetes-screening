...
x_base, y_mean, y_lower, y_upper = metric_utils.mean_curve(miss_rates, pos_rates)
plot_curve(x_base, y_mean, ylim=(0, 1), name="LGBM (no-lab)")
plot_range(x_base, y_lower, y_upper)
y_means["LGBM (no-lab)"] = y_mean

y_base, x_mean = metric_utils.mean_curve(pos_rates, miss_rates)[:2]
...
pos_rate, miss_rate = mean_pos_missrate(cv_y_prob)
plt.plot((0, 1), (pos_rate, pos_rate), color="gray", lw=1, linestyle="--")
plt.plot((miss_rate, miss_rate), (0, 1), color="gray", lw=1, linestyle="--")
plt.scatter(miss_rate, pos_rate, marker="s", color="dodgerblue")
print(f"ADA (no-lab): positive: {pos_rate}, miss_rate: {miss_rate}")
...
plot_curve(
    ...
    ylabel="Patients need examination",
)
pos_rate, miss_rate = mean_pos_missrate(cv_y_prob)
plt.plot((0, 1), (pos_rate, pos_rate), color="gray", lw=1, linestyle="--")
plt.plot((miss_rate, miss_rate), (0, 1), color="gray", lw=1, linestyle="--")
plt.scatter(miss_rate, pos_rate, marker="s", color="darkgreen")
print(f"CDS (no-lab): positive: {pos_rate}, miss_rate: {miss_rate}")
y_means["CDS (no-lab)"] = y_mean

y_base, x_mean = metric_utils.mean_curve(pos_rates, miss_rates)[:2]
x_means["CDS (no-lab)"] = x_mean

fig.savefig(os.path.join(cfg.root, "data/results/pos_missrate_withoutFPG.pdf"))
...
plt.plot((miss_rate, miss_rate), (0, 1), color="gray", lw=1, linestyle="--")
plt.scatter(miss_rate, pos_rate, marker="s", color="dodgerblue")
print(f"ADA: positive: {pos_rate}, miss_rate: {miss_rate}")
y_means["ADA"] = y_mean
...
plt.plot((miss_rate, miss_rate), (0, 1), color="gray", lw=1, linestyle="--")
plt.scatter(miss_rate, pos_rate, marker="s", color="darkgreen")
print(f"CDS: positive: {pos_rate}, miss_rate: {miss_rate}")
y_means["CDS"] = y_mean

y_base, x_mean = metric_utils.mean_curve(pos_rates, miss_rates)[:2]
x_means["CDS"] = x_mean

fig.savefig(os.path.join(cfg.root, "data/results/pos_missrate_withFPG.pdf"))


# %%
df_ymeans = pd.DataFrame(y_means.values(), index=y_means.keys(), columns=x_base)
output = os.path.join(cfg.root, "data/results/pos_missrate.csv")
os.makedirs(os.path.dirname(output), exist_ok=True)
df_ymeans.to_csv(output)

df_xmeans = pd.DataFrame(x_means.values(), index=x_means.keys(), columns=y_base)
output = os.path.join(cfg.root, "data/results/pos_missrate_transpose.csv")
os.makedirs(os.path.dirname(output), exist_ok=True)
